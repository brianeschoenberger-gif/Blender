...
def _build_half_cabin(
    ...
    vertices: list[tuple[float, float, float]] = []
    faces: list[tuple[int, int, int, int]] = []
    cabin_stations = (0.26, 0.39, 0.52, 0.64)
    row_count = 3

    for station_index, t in enumerate(cabin_stations):
        x = params.x_at(t)
        base_z = params.deck_z_at(t)
        width = max(params.cabin_half_width_at(t), 0.32)
        roof_shape = (0.46, 0.76, 0.82, 0.56)[station_index]

        vertices.extend(
            [
                (x, 0.0, base_z + params.cabin_roof_height * roof_shape),
                (x, width * 0.50, base_z + params.cabin_roof_height * (roof_shape * 0.82)),
                (x, width, base_z + 0.06),
            ]
        )
...
def _build_half_canopy(
    ...
    canopy_start_x = params.stern_x() + 1.16
    canopy_length = 1.62
    row_count = 2
    ...
    for section_t in section_ts:
        ...
        half_width = max(params.cabin_half_width_at(hull_t) * 1.06, 0.64)
        deck_z = params.deck_z_at(hull_t)
        crown = 0.025 + (0.03 if section_t < 0.75 else 0.012)

        vertices.extend(
            [
                (x, 0.0, deck_z + params.canopy_top_height + crown),
                (x, half_width, deck_z + params.canopy_top_height),
            ]
        )
...
def _build_half_window_band(
    ...
    vertices: list[tuple[float, float, float]] = []
    faces: list[tuple[int, int, int, int]] = []
    section_ts = (0.29, 0.40, 0.51, 0.62)
    row_count = 2

    for t in section_ts:
        x = params.x_at(t)
        width = max(params.cabin_half_width_at(t) * 1.00, 0.32)
        base_z = params.deck_z_at(t) + 0.20
        top_z = base_z + 0.15
...
def _build_half_windshield(
    ...
    width = max(params.cabin_half_width_at(front_t) * 0.88, 0.40)
    base_z = params.deck_z_at(front_t) + 0.30

    vertices = [
        (front_x + 0.12, width * 0.24, base_z + 0.20),
        (front_x - 0.09, width * 0.72, base_z + 0.17),
        (front_x - 0.09, width * 0.76, base_z),
        (front_x + 0.12, width * 0.28, base_z + 0.02),
    ]
    faces = [(0, 1, 2, 3)]
    return vertices, faces
...
def build_deckhouse(params: SailboatParams, context: dict) -> dict[str, bpy.types.Object]:
    ...
    frame_specs = [
        (
            ...
            [
                (params.stern_x() + 1.20, 0.68, params.deck_z_at(0.18) + 0.12),
                (params.stern_x() + 1.25, 0.72, params.deck_z_at(0.18) + 0.58),
                (params.stern_x() + 1.27, 0.66, params.deck_z_at(0.18) + params.canopy_top_height),
            ],
        ),
        (
            "CanopyRearFrame_Starboard",
            [
                (params.stern_x() + 2.14, 0.72, params.deck_z_at(0.25) + 0.07),
                (params.stern_x() + 2.20, 0.76, params.deck_z_at(0.25) + 0.56),
                (params.stern_x() + 2.22, 0.68, params.deck_z_at(0.25) + params.canopy_top_height - 0.01),
            ],
        ),
        (
            ...
            [
                (params.stern_x() + 1.27, 0.66, params.deck_z_at(0.18) + params.canopy_top_height),
                (params.stern_x() + 1.68, 0.72, params.deck_z_at(0.22) + params.canopy_top_height + 0.03),
                (params.stern_x() + 2.22, 0.68, params.deck_z_at(0.25) + params.canopy_top_height - 0.01),
            ],
        ),
    ]
    ...
    aft_span_points = [
        (params.stern_x() + 2.14, -0.68, params.deck_z_at(0.25) + params.canopy_top_height - 0.01),
        (params.stern_x() + 2.20, 0.0, params.deck_z_at(0.25) + params.canopy_top_height + 0.02),
        (params.stern_x() + 2.14, 0.68, params.deck_z_at(0.25) + params.canopy_top_height - 0.01),
    ]
    create_curve_object(
        "CanopyAftSpan",
